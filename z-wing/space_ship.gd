...
@export var f_speed = 1500.0 #forward speed
var roll_speed = 0.0
var pitch_speed = 0.0
var yaw_speed = 0.0
@export var roll_rate = 2.5
@export var roll_increase = 0.1
@export var pitch_up_rate = 1.0
@export var pitch_up_increase = 0.01
@export var pitch_down_rate = 1.1
@export var pitch_down_increase = 0.0075
@export var yaw_rate = 5.0
@export var yaw_increase = 0.01
@export var combined_speed = 900.0 #real top speed
@export var top_speed = 3000.0 #must fix!
@export var ideal_speed = 1500.0
...
func _ready():
#	$"B-wing/Cockpit/Cube3".mesh.material.albedo_color = Color8(255, 255, 255)
	pass
...
#roll
	...
	if Input.is_action_pressed("roll left"):
		...
		if roll_speed < max_roll_left:
			roll_speed += roll_increase * roll_left
		if roll_speed > max_roll_left:
			roll_speed -= 0.01
	else:
		if roll_speed > 0:
	...
	if Input.is_action_pressed("roll right"):
		if roll_speed > max_roll_right:
			roll_speed -= roll_increase * roll_right
		if roll_speed < max_roll_right:
			roll_speed += 0.01
	else:
		if roll_speed < 0:
...
#pitch
	...
	if Input.is_action_pressed("pitch up"):
		if pitch_speed < max_pitch_up:
			pitch_speed += pitch_up_increase * pitch_up
		if pitch_speed > max_pitch_up:
			pitch_speed -= 0.01
	else:
		if pitch_speed > 0:
	...
	if Input.is_action_pressed("pitch down"):
		if pitch_speed > max_pitch_down:
			pitch_speed -= pitch_down_increase * pitch_down
		if pitch_speed < max_pitch_down:
			pitch_speed += 0.01
	else:
		if pitch_speed < 0:
...
#yaw
	...
	if Input.is_action_pressed("yaw left"):
		if yaw_speed < max_yaw_left:
			yaw_speed += yaw_increase * yaw_left
		if yaw_speed > max_yaw_right:
			yaw_speed -= 0.0001
	else:
		if yaw_speed > 0:
			yaw_speed -= yaw_increase
...
#yaw gets to max yaw right faster than it gets to max yaw left, must fix
	...
	if Input.is_action_pressed("yaw right"):
		if yaw_speed > max_yaw_right:
			yaw_speed -= yaw_increase * yaw_right
		if yaw_speed < max_yaw_right:
			yaw_speed += yaw_increase
	else:
		if yaw_speed < 0:
...
#new throttle code
	...
	f_speed += ((throttle/50) - 0.05)
	f_speed -= (f_speed / ideal_speed)
...
#camera inertia
	...
	$Head/Camera3D/ShakerComponent3D.intensity = 0.001 * (f_speed / 200)
